      ...
      <table class="table table-striped">
        ...
        <tbody>
          <tr v-for="adRequest in adRequests" :key="adRequest.id">
            ...
            <td>
              {{ adRequest.by_sponsor ? "Received" : "Sent" }}
            </td>
            <td>
              <!-- Actions for Received Requests (Sent by Sponsor) -->
              <template v-if="adRequest.by_sponsor">
                <button
              ...
              <!-- Actions for Sent Requests (Sent by Influencer) -->
              <template v-if="!adRequest.by_sponsor">
                <button
                  v-if="adRequest.status === 'Pending'"
  ...
  methods: {
    async fetchAdRequests() {
      try {
        ...
        const response = await axios.get("http://localhost:5000/influencer/ad_request", {
          headers: {
    ...
    editAdRequest(adRequestId) {
      this.$router.push(`/influencer/ad_request/edit/${adRequestId}`);
    },
    deleteAdRequest(adRequestId) {
      if (confirm("Are you sure you want to delete this ad request?")) {
        try {
          const token = localStorage.getItem("access_token");
          axios
            .delete(`http://localhost:5000/influencer/ad_requests/${adRequestId}`, {
              headers: {
                Authorization: `Bearer ${token}`,
              },
            })
    ...
    viewAdRequest(adRequestId) {
      this.$router.push(`/influencer/ad_request/${adRequestId}`);
    },
  },
};
</script>
